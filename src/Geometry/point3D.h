...
public:
	...
	/**
	 * Constructs point as voxel center.
	 */
	point3D(voxel const & v) :
...
struct hash<point3D> {
	std::size_t operator()(const point3D& p) const {
		...
		// Start with a hash value of 42.
		std::size_t seed = 42;

		// Modify 'seed' by XORing and bit-shifting in
		// one member of 'Key' after the other:
		hash_combine(seed, hash<double>()(p.x));
		hash_combine(seed, hash<double>()(p.y));
		hash_combine(seed, hash<double>()(p.z));
